...
//				$this->variations_fields[] = 'pattribute_'.$att->id;
	...
	public function loadTranslations(&$arr)
	{
		...
		// Hints translations
		$arr['trans_hint_filter_id'] = __(
			'The \"filter expression\" option enables filtering using an advanced expression that allows specifying a ' .
			'mix of IDs and range of IDs. For example:<br /><br />' .
			'234,345,450-576,800,900-1005<br /><br />' .
			'This will filter IDs 234,345 and 800 and also the range of IDs between 450 and 576 and the range ' .
...
//		}
		...
		{
			$upload_dir = trim($upload_dir['baseurl']);
...
//				}
			...
			//echo 'W3Ex._translate_strings["trans_hint_filter_id"] = "'.self::relpaceInvalid($arrTranslated['trans_hint_filter_id']).'";'; echo PHP_EOL;
			echo 'W3Ex._translate_strings["trans_hint_filter_id"] = "'.$arrTranslated['trans_hint_filter_id'].'";'; echo PHP_EOL;
...
//								$woo_tags = get_terms( 'product_tag', $args );
				 ...
				 <div style="display: inline-block;position: relative;width:320px;line-height:29px;">
					 ...
					 <template id="btn-info-save-changes-body">
						<?php //_e("Changes are saved on going to a different page of products, adding products/variations or via the 'Save Changes' button","woocommerce-advbulkedit"); ?>
						 <?php _e('Changes are saved on the following actions:', 'woocommerce-advbulkedit'); ?>
						 <ul>
							 <li><?php _e('- Going to a different page of products', 'woocommerce-advbulkedit'); ?></li>
							 <li><?php _e('- Adding products/variations', 'woocommerce-advbulkedit'); ?></li>
							 <li><?php _e("- Clicking the 'Save Changes' button", 'woocommerce-advbulkedit'); ?></li>
						 </ul>
					 </template>
				 </div>
			</div>
			...
			<?php
				if (file_exists( __DIR__.'/integrations/copy-paste-variation-prices.php')) {
                    ...
                    W3ExABulkEdit_Integ_CopyPasteVariationPrices::addButtonCopy();
                    W3ExABulkEdit_Integ_CopyPasteVariationPrices::addButtonPaste();
					echo "<br/><br/>";
				}
			?>
			...
			<div style="position: relative;">
				...
				<template id="btn-info-linked-editing-body">
					<?php _e( 'When enabled, any change you make on the grid to a single product, will apply to all selected products.<br/><br/>When disabled, the change will apply only to the edited product.', 'woocommerce-advbulkedit'); ?>
				</template>
			</div>
			<br />
		...
		if(isset($settings['debugmode']))
		{
			if($settings['debugmode'] == 1)
			{
				if(function_exists('memory_get_usage'))
				{
					...
					echo 'Memory usage: '.round($usage /(1024 * 1024),2).'M';
				}
			}
		}?>
		</div>
