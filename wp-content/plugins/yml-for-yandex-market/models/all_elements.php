...
function yfym_all_elements($postId, $numFeed='1') {	
 yfym_error_log('FEED № '.$numFeed.'; Стартовала yfym_all_elements. $postId = '.$postId.'; Файл: all_elements.php; Строка: '.__LINE__, 0);	
 ...
 if ($product == null) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к get_post вернула null; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}

 if ($product->is_type('grouped')) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к сгруппированный; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 
 // что выгружать
 if ($product->is_type('variable')) {
	$yfym_whot_export = yfym_optionGET('yfym_whot_export', $numFeed, 'set_arr');
	if ($yfym_whot_export === 'simple') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к вариативный; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 }
 
 if (get_post_meta($postId, 'yfymp_removefromyml', true) === 'on')  {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен принудительно; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 $skip_flag = apply_filters('yfym_skip_flag', $skip_flag, $postId, $product, $numFeed); /* c версии 3.2.6 */
 if ($skip_flag === true) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен по флагу; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 ...
 if ($store === false || $store == '') {
	yfym_error_log('FEED № '.$numFeed.'; WARNING: Товар с postId = '.$postId.' вернул пустой $result_yml_store; Файл: all_elements.php; Строка: '.__LINE__, 0);
	$result_yml_store = '';
 } else {
 ...
 if ($delivery === false || $delivery == '') {
	yfym_error_log('FEED № '.$numFeed.'; WARNING: Товар с postId = '.$postId.' вернул пустой $delivery; Файл: all_elements.php; Строка: '.__LINE__, 0);
	$result_yml_delivery = '';
 } else {
 ...
 */	 
 ...
 if ($pickup === false || $pickup == '') {
	yfym_error_log('FEED № '.$numFeed.'; WARNING: Товар с postId = '.$postId.' вернул пустой $pickup; Файл: all_elements.php; Строка: '.__LINE__, 0);
	$result_yml_pickup = '';
 } else {
 ...
 if (class_exists('WPSEO_Primary_Term')) {		  
	...
	if ($catidWPSEO !== false) { 
	 ...
	 if ($termini !== false) {
	  ...
	  yfym_error_log('FEED № '.$numFeed.'; Warning: Для товара $postId = '.$postId.' get_the_terms = false. Возможно база битая. Пробуем задействовать wp_get_post_terms; Файл: all_elements.php; Строка: '.__LINE__, 0);$product_cats = wp_get_post_terms($postId, 'product_cat', array("fields" => "ids" ));	  
	  ...
	  if (is_array($product_cats) && count($product_cats)) {
		...
		yfym_error_log('FEED № '.$numFeed.'; Warning: Для товара $postId = '.$postId.' база наверняка битая. wp_get_post_terms вернула массив. $catpostid = '.$catpostid.'; Файл: all_elements.php; Строка: '.__LINE__, 0);
	  }
	 }
	}
 } else {	
	...
	if ($termini !== false) {
	 ...
	 yfym_error_log('FEED № '.$numFeed.'; Warning: Для товара $postId = '.$postId.' get_the_terms = false. Возможно база битая. Пробуем задействовать wp_get_post_terms; Файл: all_elements.php; Строка: '.__LINE__, 0);
 ...
 if ($result_yml_cat == '') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет категорий; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 ...
 */	
 ...
 $yfym_delivery_weekday_yml = '';
	...
	if (get_post_meta($postId, '_yfym_recommend_stock_data_arr', true) !== '') {
		$yfym_recommend_stock_data_arr = get_post_meta($postId, '_yfym_recommend_stock_data_arr', true);
		...
		$box_count = apply_filters('yfym_box_count_filter', $box_count, $product->get_id(), $product, $numFeed);		
		if ($box_count !== '') {
 ...
 if ($product->is_type('variable')) {
	yfym_error_log('FEED № '.$numFeed.'; У нас вариативный товар. Файл: all_elements.php; Строка: '.__LINE__, 0);	
	...
	for ($i = 0; $i<$variation_count; $i++) {
		...
		*/
		...
		if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет цены; Файл: all_elements.php; Строка: '.__LINE__, 0); continue;}
		...
		if ($yfym_skip_missing_products === 'on') {
			if ($offer->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к ее нет в наличии; Файл: all_elements.php; Строка: '.__LINE__, 0); continue;}
		}
		...
		if ($skip_backorders_products === 'on') {
		 if ($offer->get_manage_stock() == true) { // включено управление запасом
			if (($offer->get_stock_quantity() < 1) && ($offer->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ и включено управление запасом; Файл: all_elements.php; Строка: '.__LINE__, 0); continue;}
		 } else {
			if ($offer->get_stock_status() === 'onbackorder') { // предзаказ на уровнее вариации включен
				yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к запрещен предзаказ на уровне вариации и отключено управление запасом; Файл: all_elements.php; Строка: '.__LINE__, 0); continue;
			}
		 }
		}
		...
		if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {	  
			yfym_error_log('FEED № '.$numFeed.'; Вариация товара с postId = '.$postId.' пропущена т.к нет картинки даже в галерее; Файл: all_elements.php; Строка: '.__LINE__, 0); continue; /*continue;*/  
		}

		$skip_flag = apply_filters('yfym_skip_flag_variable', $skip_flag, $postId, $product, $offer, $numFeed); /* c версии 3.2.6 */
		if ($skip_flag === true) {yfym_error_log('FEED № '.$numFeed.'; Вариативный товар с postId = '.$postId.', offer_id = '.$offer_id.' пропущен по флагу; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
		if ($skip_flag === 'continue') {yfym_error_log('FEED № '.$numFeed.'; Вариация товара с с postId = '.$postId.', offer_id = '.$offer_id.' пропущена по флагу; Файл: all_elements.php; Строка: '.__LINE__, 0); $skip_flag = false; continue;}   
		...
		*/
		...
		if (empty($no_group_id_arr)) {
			...
			if (in_array($CurCategoryId, $no_group_id_arr)) {
			 ...
			 foreach ($attributes as $param) {					
				...
				if ($param_at_name == '') {
					yfym_error_log('FEED № '.$numFeed.'; Пропускаем товар т.к. нет атрибутов вариаций для уникальзации заголовков. Файл: all_elements.php; Строка: '.__LINE__, 0);	
		...
		$result_yml .= '<offer '.$offer_type.$gi.' id="'.$offer_id.'"'.$available_yml.$append_offer_tag.'>'.PHP_EOL;	
		do_action('yfym_prepend_variable_offer');

		$yfym_auto_disabled = yfym_optionGET('yfym_auto_disabled', $numFeed, 'set_arr');
		if ($yfym_auto_disabled === 'yes') {
			if ($offer->is_in_stock() == false) {
				$result_yml .= '<disabled>true</disabled>'.PHP_EOL;
			} else {
				$result_yml .= '<disabled>false</disabled>'.PHP_EOL;
			}
		}
		...
		if (!empty($yfym_period_of_validity_days) && $yfym_period_of_validity_days !== 'disabled') {
			$yfym_period_of_validity_days = (int)$yfym_period_of_validity_days;
			$yfym_period_of_validity_days_yml = $offer->get_attribute(wc_attribute_taxonomy_name_by_id($yfym_period_of_validity_days));
			if (!empty($yfym_period_of_validity_days_yml)) {	
				$result_yml .= "<period-of-validity-days>".strtoupper(yfym_replace_decode($yfym_period_of_validity_days_yml))."</period-of-validity-days>".PHP_EOL;		
			} else {
				$yfym_period_of_validity_days_yml = $product->get_attribute(wc_attribute_taxonomy_name_by_id($yfym_period_of_validity_days));
				if (!empty($yfym_period_of_validity_days_yml)) {	
					$result_yml .= "<period-of-validity-days>".strtoupper(yfym_replace_decode($yfym_period_of_validity_days_yml))."</period-of-validity-days>".PHP_EOL;		
				}		
			}
		...
		$result_yml .= $result_availability_yml;
		$result_yml .= $result_transport_unit_yml;
		$result_yml .= $result_min_delivery_pieces_yml;
		$result_yml .= $result_quantum_yml;
		$result_yml .= $result_leadtime_yml;
		$result_yml .= $result_box_count_yml;
	...
	yfym_error_log('FEED № '.$numFeed.'; Все вариации выгрузили. '.$ids_in_yml.' Файл: all_elements.php; Строка: '.__LINE__, 0);	
	
	return array($result_yml, $ids_in_yml); // все вариации выгрузили	
 } // end if ($product->is_type('variable'))	 
 /* end Вариации */

 yfym_error_log('FEED № '.$numFeed.'; У нас обычный товар. Файл: all_elements.php; Строка: '.__LINE__, 0);
 ...
 if ($price_yml == 0 || empty($price_yml)) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет цены; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 if (class_exists('YmlforYandexMarketPro')) {
	if ((yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== false) && (yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr') !== '')) {
		$yfymp_compare_value = yfym_optionGET('yfymp_compare_value', $numFeed, 'set_arr');
		$yfymp_compare = yfym_optionGET('yfymp_compare', $numFeed, 'set_arr');			 
 ...
 $yfym_skip_missing_products = yfym_optionGET('yfym_skip_missing_products', $numFeed, 'set_arr');
 yfym_error_log('FEED № '.$numFeed.'; $yfym_skip_missing_products = '.$yfym_skip_missing_products.'; gettype = '.gettype($yfym_skip_missing_products).'; Файл: all_elements.php; Строка: '.__LINE__, 0);
 if ($yfym_skip_missing_products === 'on') {
	if ($product->is_in_stock() == false) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет в наличии; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml;}
 }		  
 ...
 if ($skip_backorders_products === 'on') {
	if ($product->get_manage_stock() == true) { // включено управление запасом  
		if (($product->get_stock_quantity() < 1) && ($product->get_backorders() !== 'no')) {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ и включено управление запасом; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	} else {
		if ($product->get_stock_status() !== 'instock') {yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к запрещен предзаказ; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/}
	}
 }  
 ...
 if (($yfym_skip_products_without_pic === 'on') && ($picture_yml == '')) {	  
	yfym_error_log('FEED № '.$numFeed.'; Товар с postId = '.$postId.' пропущен т.к нет картинки даже в галерее; Файл: all_elements.php; Строка: '.__LINE__, 0); return $result_yml; /*continue;*/  
 }
 ...
 $result_yml .= '<offer '.$offer_type.' id="'.$postId.'"'.$available_yml.$append_offer_tag.'>'.PHP_EOL;
